...
### Popular Hypothesis testing (when the data are normally distributed)
 ...
 - f)Comparing `**multiple samples**` (ANOVA Test, the involvement of categorical variables).
   ...
   - Two-Way(factor) ANOVA Test (one response VS two predictors with multiple(`>=3`) classes)
     ...
     - [Notice]: We have 2 categorical variables, but we should generate **2** integrated categorical variables. In order to do this, note that `one of two original categorical variables cannot hold more than binary classes` because it's a two-way ANOVA. This binary class becomes `[World-I]` & `[World-II]`.
